...
buffer_type zynq_buffer::type(buftype_ZYNQ{});
...
void zynq_buffer::post_work(int nitems)
{
    ...
    msg << "[" << this << "] "
        << "zynq_buffer [" << d_context << "] -- post_work: " << nitems;
                                         ...
                                         unsigned read_index)
{
    ...
    std::ostringstream msg;
    msg << "[" << this << "] "
        << "zynq_buffer [" << d_context << "] -- input_blocked_callback";
    GR_LOG_DEBUG(d_logger, msg.str());
#endif
...
bool zynq_buffer::output_blocked_callback(int output_multiple, bool force)
{
    ...
    std::ostringstream msg;
    msg << "[" << this << "] "
        << "zynq_buffer [" << d_context << "] -- output_blocked_callback";
    GR_LOG_DEBUG(d_logger, msg.str());
#endif
...
buffer_sptr zynq_buffer::make_zynq_buffer(int nitems,
                                          size_t sizeof_item,
                                          uint64_t downstream_lcm_nitems,
                                          block_sptr link,
                                          block_sptr buf_owner)
{
    return buffer_sptr(
        new zynq_buffer(nitems, sizeof_item, downstream_lcm_nitems, link, buf_owner));
}
