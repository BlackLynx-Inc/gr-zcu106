...
proc init_gui { IPINST   } {
  ...
  ipgui::add_row $IPINST -parent ${gui_text_basic}
  #ipgui::add_dynamic_text $IPINST -name "ENCODER_BW_ESTIMATION2" -tclproc encoder_bw_calc -display_border false -parent ${gui_text_advanced}
  ...
  ipgui::add_row $IPINST -parent $encoder_config
     set encoder_param  [ipgui::add_group $IPINST -parent $encoder_config -name "Stream Details - Effective" -layout horizontal]  
     ...
     set_property tooltip "Video Resolution of incoming stream. Where multiple streams are to be used see documentation to understand how to set effective max." [ipgui::get_guiparamspec  ENC_FRAME_SIZE -of $IPINST]
     ...
     set_property tooltip "Maximum effective frame rate of input stream(s). Where multiple streams are to be used see documentation to understand how to set effective max." [ipgui::get_guiparamspec  ENC_FPS -of $IPINST]
  ...
  ipgui::add_row $IPINST -parent $decoder_config
    set decoder_param [ipgui::add_group $IPINST -parent $decoder_config -name "Stream Details - Effective" -layout horizontal]  
    ...
    set_property tooltip  "Video Resolution of incoming encoded stream. Where multiple streams are to be used see documentation to understand how to set effective max." [ipgui::get_guiparamspec  DEC_FRAME_SIZE -of $IPINST]
    set_property tooltip  "Maximum effective frame rate of stream(s) to be decoded. Where multiple streams are to be used see documentation to understand how to set effective max." [ipgui::get_guiparamspec  DEC_FPS -of $IPINST]
...
proc init_params {IPINST PARAM_VALUE.ENABLE_ENCODER  PARAM_VALUE.ENC_CODING_TYPE PARAM_VALUE.ENC_BUFFER_EN PARAM_VALUE.ENC_BUFFER_TYPE  PARAM_VALUE.ENC_BUFFER_SIZE PARAM_VALUE.ENC_BUFFER_MANUAL_OVERRIDE  PARAM_VALUE.ENC_BUFFER_B_FRAME  PARAM_VALUE.ENC_BUFFER_MOTION_VEC_RANGE  PARAM_VALUE.WPP_EN    } {
...
#######  Encoder BW Calculation #########################
proc encoder_bw_calc { ENC_CODING_TYPE ENC_VIDEO_STANDARD ENC_COLOR_DEPTH ENC_COLOR_FORMAT ENC_FRAME_SIZE ENABLE_ENCODER ENC_FPS CORE_CLK ENC_BUFFER_EN  ENC_BUFFER_B_FRAME } {
...
proc encoder_depth_calc {PARAM_VALUE.ENC_BUFFER_SIZE PARAM_VALUE.ENC_CODING_TYPE  PARAM_VALUE.ENC_VIDEO_STANDARD PARAM_VALUE.ENC_COLOR_DEPTH PARAM_VALUE.ENC_COLOR_FORMAT PARAM_VALUE.ENC_FRAME_SIZE PARAM_VALUE.ENABLE_ENCODER PARAM_VALUE.ENC_FPS PARAM_VALUE.CORE_CLK PARAM_VALUE.ENC_BUFFER_EN  PARAM_VALUE.ENC_BUFFER_B_FRAME PARAM_VALUE.ENC_MEM_BRAM_USED  PARAM_VALUE.ENC_MEM_URAM_USED } {
     #send_msg INFO 201 "in calcu BW  TOTAL_BYTES "
	 ...
	 set ENC_BUFFER_EN [get_property value ${PARAM_VALUE.ENC_BUFFER_EN}]
	 set ENC_BUFFER_B_FRAME [get_property value ${PARAM_VALUE.ENC_BUFFER_B_FRAME}]
     set ret_str [encoder_bw_calc $ENC_CODING_TYPE  $ENC_VIDEO_STANDARD  $ENC_COLOR_DEPTH   $ENC_COLOR_FORMAT   $ENC_FRAME_SIZE $ENABLE_ENCODER  $ENC_FPS  $CLOCK  $ENC_BUFFER_EN  $ENC_BUFFER_B_FRAME ]
	 #set ret_str "aaa"
     ...
     return "Encoder Buffer Size - $ENC_BUFFER_SIZE \n $ret_str \n"
     #return "Encoder Buffer Size - $ENC_BUFFER_SIZE / $TOTAL_MEM KBytes \n $ret_str \n"
     #return "Encoder Buffer Size - $TOTAL_MEM KBytes \n "
}
...
proc l2c_depth_calc { FRAME_SIZE FPS CLOCK COLOR_FORMAT ENC_BUFFER_B_FRAME VERTICAL_RANGE VIDEO_STANDARD} {
...
proc update_PARAM_VALUE.ENC_BUFFER_SIZE_ACTUAL { PARAM_VALUE.ENC_BUFFER_TYPE PARAM_VALUE.ENC_BUFFER_B_FRAME PARAM_VALUE.ENC_VIDEO_STANDARD PARAM_VALUE.CORE_CLK  PARAM_VALUE.ENC_FPS PARAM_VALUE.ENC_CODING_TYPE PARAM_VALUE.ENC_BUFFER_MANUAL_OVERRIDE PARAM_VALUE.ENABLE_ENCODER PARAM_VALUE.ENC_BUFFER_SIZE_ACTUAL PARAM_VALUE.ENC_COLOR_DEPTH PARAM_VALUE.ENC_BUFFER_MOTION_VEC_RANGE PARAM_VALUE.ENC_FRAME_SIZE PARAM_VALUE.ENC_COLOR_FORMAT PARAM_VALUE.ENC_BUFFER_EN } {
	...
	set L2C_Depth [ l2c_depth_calc  $FRAME_SIZE $FPS $CLOCK $COLOR_FORMAT $ENC_BUFFER_B_FRAME $VERTICAL_RANGE $VIDEO_STANDARD ]

    if { $ENABLE_ENCODER == "false" || $ENABLE_ENCODER == "FALSE" || $ENC_CODING_TYPE == 0 || $ENC_BUFFER_EN == "false" || $ENC_BUFFER_EN == "FALSE" } {
 	   set_property value 0 ${PARAM_VALUE.ENC_BUFFER_SIZE_ACTUAL}
	} else {
 	   set_property value [expr ((($L2C_Depth * $MULTI_ACTUAL) + (8*1024) -1)/(8*1024))] ${PARAM_VALUE.ENC_BUFFER_SIZE_ACTUAL}
	}

}


proc update_PARAM_VALUE.ENC_BUFFER_SIZE { PARAM_VALUE.ENC_BUFFER_TYPE PARAM_VALUE.ENC_BUFFER_B_FRAME PARAM_VALUE.ENC_VIDEO_STANDARD PARAM_VALUE.CORE_CLK  PARAM_VALUE.ENC_FPS PARAM_VALUE.ENC_CODING_TYPE PARAM_VALUE.ENC_BUFFER_MANUAL_OVERRIDE PARAM_VALUE.ENABLE_ENCODER PARAM_VALUE.ENC_BUFFER_SIZE PARAM_VALUE.ENC_COLOR_DEPTH PARAM_VALUE.ENC_BUFFER_MOTION_VEC_RANGE PARAM_VALUE.ENC_FRAME_SIZE PARAM_VALUE.ENC_COLOR_FORMAT PARAM_VALUE.ENC_BUFFER_EN } {
    ...
    if { $ENABLE_ENCODER == "false" || $ENABLE_ENCODER == "FALSE" || $ENC_CODING_TYPE == 0 || $ENC_BUFFER_EN == "false" || $ENC_BUFFER_EN == "FALSE" } {
 	   ...
 	   set_property value [expr ((($L2C_Depth * $MULT) + (8*1024) -1)/(8*1024))] ${PARAM_VALUE.ENC_BUFFER_SIZE}
 	   #set_property value [expr ((($L2C_Depth * $MULT_ACTUAL) + (8*1024) -1)/(8*1024))] ${PARAM_VALUE.ENC_BUFFER_SIZE_ACTUAL}
	}
...
proc update_MODELPARAM_VALUE.HDL_PLL_CLK { PARAM_VALUE.PLL_CLK MODELPARAM_VALUE.HDL_PLL_CLK} {
	set PLL_CLK [get_property value ${PARAM_VALUE.PLL_CLK}]
    set_property value ${PLL_CLK} ${MODELPARAM_VALUE.HDL_PLL_CLK}
}
...
proc update_MODELPARAM_VALUE.HDL_AXI_MCU_CLK {PARAM_VALUE.AXI_MCU_CLK MODELPARAM_VALUE.HDL_AXI_MCU_CLK } {
   set_property value [get_property value ${PARAM_VALUE.AXI_MCU_CLK}] ${MODELPARAM_VALUE.HDL_AXI_MCU_CLK}
}

proc update_MODELPARAM_VALUE.HDL_AXI_DEC_CLK {PARAM_VALUE.AXI_DEC_CLK MODELPARAM_VALUE.HDL_AXI_DEC_CLK } {
   set_property value [get_property value ${PARAM_VALUE.AXI_DEC_CLK}] ${MODELPARAM_VALUE.HDL_AXI_DEC_CLK}
}

proc update_MODELPARAM_VALUE.HDL_AXI_DEC_CLK {PARAM_VALUE.AXI_ENC_CLK MODELPARAM_VALUE.HDL_AXI_ENC_CLK } {
   set_property value [get_property value ${PARAM_VALUE.AXI_ENC_CLK}] ${MODELPARAM_VALUE.HDL_AXI_ENC_CLK}
}
